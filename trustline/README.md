...
This project implements the [Consents API specification](docs/consentsAPI.yaml) following the [case instructions](docs/case-instructions.md): a Java service that creates, reads, updates and revokes consents, with the HTTP contract fixed by the provided OpenAPI file.

Beyond the required API, the submission includes automated tests, GitHub Actions workflows for CI and releases, and an infrastructure-as-code definition that deploys the service to AWS.

> The original submission also included a walkthrough video and a mind map board. They were left out of this repository because they were not anonymized.
...
At submission time the service was deployed to AWS with the infrastructure described above, so the reviewers could exercise a production-like environment. That deployment has since been torn down.
