...
A NestJS API implementing the challenge below: a complete user CRUD backed by MongoDB, a Brazilian postal-code (CEP) lookup, a paginated search over the public PokeAPI, and a Redis cache in front of both external services. The whole environment (API, MongoDB and Redis) starts with a single Docker Compose command.

## Challenge statement

Reproduced as received, in Portuguese.
...
With the project running, the Swagger UI is available at:

[http://localhost:3000/api-spec](http://localhost:3000/api-spec)

## Missing implementations

Some of the requirements were not implemented due to time restrictions:
...
The company sent a list of follow-up questions with the challenge. Questions and answers are kept as submitted, in Portuguese.
